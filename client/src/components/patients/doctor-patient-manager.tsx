...
function PatientSkeleton() {
  return <div role="status" aria-label="Loading patients"><span className="sr-only">Loading patients</span><Card className="overflow-x-auto border-0 bg-transparent py-0"><CardContent className="min-w-[52rem] space-y-4 p-5">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-8 w-full" />)}</CardContent></Card></div>
}

function PatientActions({ patient, deleting, onDelete }: { patient: Patient; deleting: boolean; onDelete: (patient: Patient) => void }) {
  return <Tooltip><TooltipTrigger render={<Button type="button" variant="ghost" size="icon-sm" className="border-red-500/25 text-red-400 hover:bg-red-500/10 hover:text-red-300" aria-label={`Delete ${patient.name}`} disabled={deleting} onClick={() => onDelete(patient)} />}><Trash2 aria-hidden="true" /></TooltipTrigger><TooltipContent>Delete patient</TooltipContent></Tooltip>
}

function PatientList({ patients, deletingId, onDelete }: { patients: Patient[]; deletingId: string | null; onDelete: (patient: Patient) => void }) {
  return <Card className="overflow-hidden border-0 bg-transparent py-0"><Table className="min-w-[52rem]"><TableHeader><TableRow><TableHead className="pl-5">Patient</TableHead><TableHead>Age / gender</TableHead><TableHead>Phone</TableHead><TableHead>Condition</TableHead><TableHead>Appointment</TableHead><TableHead className="w-20 pr-5 text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{patients.map((patient) => <TableRow key={patient.id}><TableCell className="pl-5 font-medium text-neutral-100"><div className="flex items-center gap-2.5"><Avatar size="sm"><AvatarFallback className="bg-white/15 text-[0.65rem] font-semibold text-white">{getInitials(patient.name)}</AvatarFallback></Avatar><span>{patient.name}</span></div></TableCell><TableCell><span>{patient.age}</span><Badge variant="secondary" className="ml-2 capitalize">{patient.gender}</Badge></TableCell><TableCell><a href={`tel:${patient.phone}`} className="hover:underline">{patient.phone}</a></TableCell><TableCell className="max-w-56"><Badge variant="outline" className="max-w-full truncate">{patient.condition}</Badge></TableCell><TableCell><span className="flex items-center gap-2"><CalendarDays className="size-3.5 text-neutral-500" aria-hidden="true" />{dateFormatter.format(new Date(patient.appointmentDate))}</span></TableCell><TableCell className="pr-5 text-right"><PatientActions patient={patient} deleting={deletingId === patient.id} onDelete={onDelete} /></TableCell></TableRow>)}</TableBody></Table></Card>
}
...
export function DoctorPatientManager({ doctorId, doctorName }: { doctorId: string; doctorName: string }) {
  ...
  return <section aria-labelledby="patients-heading" className="space-y-5">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-neutral-500">Care roster</p><h2 id="patients-heading" className="mt-1 text-2xl font-semibold tracking-[-0.025em]">Patients</h2><p className="mt-1 text-sm text-neutral-400">Appointments assigned to {doctorName}.</p></div><PatientFormDialog mode="create" doctorId={doctorId} doctorName={doctorName} trigger={<Button><Plus aria-hidden="true" />Add patient</Button>} /></div>
    <div className="flex min-w-0 items-center gap-2 [&_input]:h-9 [&_input]:rounded-full [&_[data-slot=button]]:size-9 [&_[data-slot=button]]:rounded-full"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search patients by name, phone, condition, or gender..." aria-label="Search patients" className="pl-9" /></div><div className="flex shrink-0 gap-2">
