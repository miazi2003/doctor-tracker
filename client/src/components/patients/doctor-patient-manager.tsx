...
export function DoctorPatientManager({ doctorId, doctorName }: { doctorId: string; doctorName: string }) {
  ...
  return <section aria-labelledby="patients-heading" className="space-y-5">
    ...
    <div className="flex flex-col gap-2 [&_input]:h-9 [&_input]:rounded-full [&_[data-slot=button]]:size-9 [&_[data-slot=button]]:rounded-full sm:flex-row"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search patients by name, phone, condition, or gender..." aria-label="Search patients" className="pl-9" /></div><div className="flex gap-2">
