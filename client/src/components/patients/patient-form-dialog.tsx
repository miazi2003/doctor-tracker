      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl" showCloseButton={!mutation.isPending}>
        ...
        <form onSubmit={form.handleSubmit(submit)} noValidate className="space-y-5">
          ...
          <div className="grid gap-x-5 gap-y-4 sm:grid-cols-2">
            <div><Label htmlFor={`${props.mode}-patient-name`}>Full name</Label><Input id={`${props.mode}-patient-name`} className="mt-2" autoComplete="name" disabled={mutation.isPending} aria-invalid={Boolean(errorFor("name"))} {...form.register("name")} />{errorFor("name") && <p className="mt-1 text-xs text-destructive">{errorFor("name")?.message}</p>}</div>
            <div><Label htmlFor={`${props.mode}-patient-age`}>Age</Label><Input id={`${props.mode}-patient-age`} type="number" min={0} max={130} inputMode="numeric" className="mt-2" disabled={mutation.isPending} aria-invalid={Boolean(errorFor("age"))} {...form.register("age")} />{errorFor("age") && <p className="mt-1 text-xs text-destructive">{errorFor("age")?.message}</p>}</div>
            <div><Label htmlFor={`${props.mode}-patient-gender`}>Gender</Label><Select value={gender} onValueChange={(value) => form.setValue("gender", value as PatientGender, { shouldDirty: true, shouldValidate: true })} disabled={mutation.isPending}><SelectTrigger id={`${props.mode}-patient-gender`} className="mt-2 w-full"><SelectValue /></SelectTrigger><SelectContent>{patientGenders.map((option) => <SelectItem key={option} value={option}><span className="capitalize">{option}</span></SelectItem>)}</SelectContent></Select>{errorFor("gender") && <p className="mt-1 text-xs text-destructive">{errorFor("gender")?.message}</p>}</div>
            <div><Label htmlFor={`${props.mode}-patient-phone`}>Phone</Label><Input id={`${props.mode}-patient-phone`} type="tel" autoComplete="tel" className="mt-2" disabled={mutation.isPending} aria-invalid={Boolean(errorFor("phone"))} {...form.register("phone")} />{errorFor("phone") && <p className="mt-1 text-xs text-destructive">{errorFor("phone")?.message}</p>}</div>
            <div><Label htmlFor={`${props.mode}-patient-condition`}>Condition</Label><Input id={`${props.mode}-patient-condition`} className="mt-2" disabled={mutation.isPending} aria-invalid={Boolean(errorFor("condition"))} {...form.register("condition")} />{errorFor("condition") && <p className="mt-1 text-xs text-destructive">{errorFor("condition")?.message}</p>}</div>
            <div><Label htmlFor={`${props.mode}-patient-appointment`}>Appointment date</Label><Input id={`${props.mode}-patient-appointment`} type="date" className="mt-2" disabled={mutation.isPending} aria-invalid={Boolean(errorFor("appointmentDate"))} {...form.register("appointmentDate")} />{errorFor("appointmentDate") && <p className="mt-1 text-xs text-destructive">{errorFor("appointmentDate")?.message}</p>}</div>
          </div>
